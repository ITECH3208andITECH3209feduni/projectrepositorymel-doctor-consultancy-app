...
<nav class="w3-sidebar w3-bar-block w3-white w3-collapse w3-top" style="z-index:3;width:250px" id="mySidebar">
  ...
  <div class="w3-padding-64 w3-large w3-text-grey" style="font-weight:bold">
    ...
    <a href="checkup.php" class="w3-bar-item w3-button">Consult</a>
	<a href="pharmacy.php" class="w3-bar-item w3-button">Pharmacy</a>
	<a href="diagnostic.php" class="w3-bar-item w3-button">Diagnostic</a>
...
<?php 
		...
		$loadQuery = "SELECT * FROM users WHERE email = '$email'";
		$result = $con->query($loadQuery);
		$rows = $result->fetch_assoc();
		$_SESSION['username'] = $rows['username'];
		$_SESSION['email'] = $rows['email'];
		$_SESSION['photo'] = $rows['photo_url'];
	?>
	
	<div class = "dashContainer"> 
		
		<div class="w3-row-padding ">
    <button onclick="window.location.href='index.php';" class = "w3-button w3-blue w3-right">Logout</button>
	<div class = " w3-right">
			    <?php
				...
				$_SESSION['user'] = "SELECT * FROM user }'";
				$outputLine = "";
				$outputLine = substr_replace($outputLine,"",-2);
				echo("<pre>".$outputLine."</pre>");
			?>
			</div>
		
			<img src = "<?php echo($_SESSION['photo'])?>" alt="user photo" "border-radius: 30%" style=" width:30%" class = "mainImg">
			</div>
	...
	<div class="list of doctor">
	
	<?php
	require ("utils/config2.php");
		require ("utils/auth.php");
		$loadQuery = "SELECT * FROM doctor";
		$result = $con->query($loadQuery);
		$rows = $result->fetch_assoc();
		$name = $rows['username'];
		$email['email'] = $rows['email'];
		$photo['photo'] = $rows['photo_url'];
