...
<a href="Signin.php" class="w3-button w3-blue w3-right">Sign in</a>
  ...
  <?php
				...
				if(isset($_POST['username'])){
					...
					$checkUserNameQuery="SELECT * FROM doctor WHERE username = '$username'";		
					...
					else
					{
						...
						$idResult = $con->query("SELECT MAX(id) AS id FROM doctor");
						$idResult1 = $idResult->fetch_assoc();
						$maxId = $idResult1['id'];
						$con->query("ALTER TABLE doctor AUTO_INCREMENT = $maxId");
						$query = "INSERT INTO doctor(username,number,email,password,address,profile,photo_url) VALUES ('$username','$number','$email','$password','$address',NULL,'$picture')";
						if($con->query($query)){
