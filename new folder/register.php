  ...
  <?php
				...
				if(isset($_POST['username'])){
					...
					else
					{
						...
						$idResult = $con->query("SELECT MAX(id) AS id FROM user");
						$idResult1 = $idResult->fetch_assoc();
						$maxId = $idResult1['id'];
						$con->query("ALTER TABLE user AUTO_INCREMENT = $maxId");
						$query = "INSERT INTO user(username,number,email,password,address,profile) VALUES ('$username','$number','$email','$password','$address',NULL)";
						if($con->query($query)){
